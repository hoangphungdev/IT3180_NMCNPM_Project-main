...
import { deleteDataExpense } from '../../database/Dao/ExpenseDao';
import { deleteDataExpensePayment } from '../../database/Dao/PaymentDao';

const DeleteExpenseModal = (props) => {
    const confirmDelete = async () => {
        const expense_id = props.expense_id
        await deleteDataExpense(expense_id);
        await deleteDataExpensePayment(expense_id);
        Alert.alert('Thông báo', 'Xóa phí dịch vụ thành công !');
        navigation.goBack();
    };
    ...
    const AndroidButtonBox = () => {
        const buttons = [
            { text: 'Hủy bỏ' },
            { text: 'Xác nhận', func: () => { confirmDelete() } }
        ];
        ...
        >
            ...
            <View style={styles.alertBox}>
                <View style={[styles.androidAlertBox, styles.container]}>
                    ...
                    <Text style={styles.message}>{'Bạn muốn xóa phí dịch vụ này ?'}</Text>
                    <AndroidButtonBox />
                </View>
            </View>
        </Modal>
